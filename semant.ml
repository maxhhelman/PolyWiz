(* Semantic checking for the PolyWiz compiler *)
  ...
  let built_in_decls =
    let add_bind map (name, ty) = StringMap.add name {
      typ = if name="new_poly" then Poly
            ...
            else if name="tex_document" then String
  ...
  let check_function func =
    ...
    let rec expr = function
      ...
      | Binop(e1, op, e2) as e ->
          ...
          | Add | Sub | Mult | Compo when same && t1 = Poly -> Poly
          | Div when t1=Poly && t2=Float -> Poly
          | Eval when t1=Poly && t2=Float -> Float
          | Exp when false==same-> Float
          | Equal | Neq            when same               -> Bool
          | Less | Leq | Greater | Geq
                     when same && (t1 = Int || t1 = Float) -> Bool
          | In when t1 = Array(t2) -> Bool
