...
# Regression testing script for PolyWiz
...
Usage() {
    echo "Usage: testall.sh [options] [.wiz files]"
...
CheckFail() {
    ...
    RunFail "$POLYWIZ" "<" $1 "2>" "${basename}.err" ">>" $globallog &&
...
then
    ...
    files="tests/test-*.wiz tests/fail-*.wiz"
fi
